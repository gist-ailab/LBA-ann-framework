...
class Canvas(QtWidgets.QWidget):
    ...
    def initializePromptModel(self, name):
        from src import auto_label
        if name == "SAM2":
            self._ai_model = auto_label.Sam2Labler()
            
        # self._ai_model.set_image(image=img_qt_to_arr(self.pixmap.toImage()))
    ...
    def mouseMoveEvent(self, ev):
        ...
        if self.drawing():
            ...
            if "prompt" in self.createMode :
                if self.createMode == "prompt-box":
                    self.line.shape_type = "rectangle"
                
                elif self.createMode == "prompt-mask":
                    self.line.shape_type = "mask"
                    
                elif self.createMode == "prompt-points":
            ...
            elif self.createMode == "Prompt_Points":
        ...
        if ev.button() == QtCore.Qt.MouseButton.LeftButton:
            if self.drawing():
                if self.current:
                    ...
                    elif "prompt" in self.createMode:
                        if self.createMode == "prompt-box":
                            print("Prompt-box mode mouse press")
                            assert len(self.current.points) == 1
                            self.current.points = self.line.points
                            self.finalise()
                    ...
                    if "prompt" in self.createMode:
                        ...
                        if self.createMode == "prompt-points":
                            self.current = Shape(shape_type="points")
                            self.current.addPoint(pos, label=0 if is_shift_pressed else 1)
                            
                            print("Prompt-point mode mouse press")
                ...
                if self.selectedShapes != []:
                    if (self.selectedShapes[0].selected == True):
                        if self.hShape not in self.selectedShapes:
                            self.selectedShapes[0].selected = False
                            self.repaint()
                            self.selectedShapes = []
                            return
                    ...
                    elif self.selectedVertex() and ev.modifiers() == (QtCore.Qt.AltModifier | QtCore.Qt.ShiftModifier):
    ...
    def mouseReleaseEvent(self, ev):
        print("mouse relesase")

        if ev.button() == QtCore.Qt.MouseButton.RightButton:
            menu = self.menus[len(self.selectedShapesCopy) > 0]
            self.restoreCursor()
            if not menu.exec_(self.mapToGlobal(ev.position().toPoint())) and self.selectedShapesCopy:
            ...
            if self.editing():
                ...
                if "prompt" in self.createMode:
                    if self.createMode == "prompt-box":
                        print("Prompt-box mode mouse relesase")
                        
                        
                    elif self.createMode == "prompt-points":
                        print("Prompt-point mode mouse relesase")
        ...
        if (
            ...
            if len(points) > 2:
                drawing_shape.setShapeRefined(
                    shape_type="polygon",
                    points=[QtCore.QPoint(point[0], point[1]) for point in points],
            ...
            drawing_shape.setShapeRefined(
                shape_type="mask",
                points=[QtCore.QPoint(x1, y1), QtCore.QPoint(x2, y2)],
                point_labels=[1, 1],
    ...
    def finalise(self):
        ...
        if "prompt" in self.createMode:
            if self.createMode == "prompt-box":
                input_points = [[p.x(), p.y()] for p in self.current.points]
                input_prompt = find_box_coordinates(input_points)
                points = self._ai_model.box2polygon(input_prompt)
                ...
                input_prompt = [[p.x(), p.y()] for p in self.current.points]
                points = self._ai_model.point2polygon(input_prompt)
        ...
        cls_idx = self.parent().window().class_list.currentRow()
        self.current.label = self.parent().window().categories_list[cls_idx]
        self.current.close()
        ...
        self.parent().window().update_shape_list()
    ...
    def keyPressEvent(self, ev):
        ...
        if self.drawing():
            ...
            if key == QtCore.Qt.Key_Up:
                ...
                self.selectedShapes[0].selected = False
                self.repaint()
                # self.selectedShapes[0].highlightClear()
                self.selectedShapes = []
